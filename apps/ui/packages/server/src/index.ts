...
export class App {
    ...
    async config (socketIO?: Server) {
        ...
        let sessionToolMap: Record<string, AgentAdd[]> = {}
        this.app.get('/api/v1/chat/session/getTool/:key', (req: Request, res: Response) => {
            const key = req.params.key
            let data = {
                sessionId: key,
                agents: sessionToolMap[key]
            }
            if (sessionToolMap[key]) {
        ...
        this.app.post('/api/v1/chat/session/addTool', async (req: Request, res: Response) => {
            const data = req.body
            const sessionId = data.sessionId
            const agent = data.agent as AgentAdd
            if (sessionToolMap[sessionId]) {
                sessionToolMap[sessionId].push(agent)
            } else {
                sessionToolMap[sessionId] = [agent]
            }
            return res.json({
                data: 'update success !'
            })
